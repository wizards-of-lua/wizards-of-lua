...
public class CommandWol extends CommandBase {
        ...
        if (TICKSLIMIT.equals(section)) {
          ...
          if (action == null) {
            sender.addChatMessage(new TextComponentString(
                "Max. Lua ticks per game tick: " + LuaMod.instance.getTicksLimit()));
            return;
          } else if (SET.equals(action)) {
            Long value = getArgAsLong(args, 2, "value");
            LuaMod.instance.setTicksLimit(value);
            return;
          }
        }
      }
  ...
  private void say(ICommandSender sender, String message) {
    ...
    sender.getEntityWorld().getMinecraftServer().getPlayerList().sendChatMsg(
