package net.karneim.luamod.lua.classes.event.entity.living;

import net.karneim.luamod.lua.classes.LuaModule;
import net.karneim.luamod.lua.classes.LuaTypesRepo;
import net.karneim.luamod.lua.util.table.DelegatingTable;
import net.karneim.luamod.lua.util.wrapper.DelegatingLuaClass;
import net.minecraftforge.event.entity.living.LivingEntityUseItemEvent;
import net.sandius.rembulan.Table;

@LuaModule("LivingEntityUseItemTickEvent")
public class LivingEntityUseItemTickEventClass
    extends DelegatingLuaClass<LivingEntityUseItemEvent.Tick> {
  public LivingEntityUseItemTickEventClass(LuaTypesRepo repo) {
    super(repo);
  }

  @Override
  protected void addProperties(DelegatingTable.Builder<? extends LivingEntityUseItemEvent.Tick> b,
      LivingEntityUseItemEvent.Tick delegate) {
    b.addReadOnly("type", () -> repo.wrap(getModuleName()));
  }

  @Override
  protected void addFunctions(Table luaClass) {}
}
