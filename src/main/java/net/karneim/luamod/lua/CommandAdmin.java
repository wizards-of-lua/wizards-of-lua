...
public class CommandAdmin extends CommandBase {
		...
		if (args.length > 0) {
			...
			if (PERMISSION.equals(args[0])) {
				...
				if (args.length <= 4 && AUTO.equals(args[1]) && SET.equals(args[2])) {
					return getListOfStringsMatchingLastWord(args, asList(Permissions.AutoWizardPermission.names()));
				}
			...
			if (TICKSLIMIT.equals(args[0])) {
				...
				if (args.length <= 3) {
					return getListOfStringsMatchingLastWord(args, asList("100", "1000", "10000", "100000", "1000000"));
				}
			}
		}
			...
			if (args.length > 0) {
				...
				if (PERMISSION.equals(section)) {
					...
					if (GRANT.equals(scope)) {
						ITextComponent message = new TextComponentString("Sorry - command currently not supported!");
						sender.addChatMessage(message);
						return;
					}
	...
	private void setGithubToken(ICommandSender sender, String token) throws CommandException {
		try {
			...
			throw new CommandException("Can't authenticate with GitHub! Caught exception with message: %s!",
					e.getMessage());
		}
	}
	...
	private void setGithubLogin(ICommandSender sender, String username, String password) throws CommandException {
		try {
			if (sender == null || sender.getCommandSenderEntity() == null) {
				throw new CommandException("Command sender is not a player!");
			}
	...
	private void clearCache(ICommandSender sender) throws CommandException {
		try {
			...
			say(sender, "Cache cleared!");
			//sender.addChatMessage(new TextComponentString("Cache cleared!"));
		} catch (IOException e) {
			throw new CommandException("Can't clear lua cache! Caught exception with message: %s!", e.getMessage());
		}
	}

	private void say(ICommandSender sender, String message) {
		TextComponentString text = new TextComponentString(message);
		sender.getEntityWorld().getMinecraftServer().getPlayerList().sendChatMsg(new TextComponentTranslation("chat.type.announcement", new Object[] {"WoL", text}));
	}
	...
	private void setUserProfileRef(ICommandSender sender, @Nullable String refStr) throws CommandException {
		try {
			Entity owner = sender.getCommandSenderEntity();
			...
			throw new CommandException("Can't load gist with id %s! Caught exception with message: %s!", refStr,
					e.getMessage());
		}
	}
	...
	private void setDefaultProfileUrl(ICommandSender sender, @Nullable String refStr) throws CommandException {
		try {
			Entity owner = sender.getCommandSenderEntity();
			GistFileRef gistFileRef = toGistFileRef(refStr);
			if (gistFileRef != null) {
	...
	private void setStartupProfileUrl(ICommandSender sender, @Nullable String refStr) throws CommandException {
		try {
			Entity owner = sender.getCommandSenderEntity();
			GistFileRef gistFileRef = toGistFileRef(refStr);
			if (gistFileRef != null) {
	...
	private @Nullable String getArg(String[] args, int idx, String name, boolean mandatory) throws CommandException {
		if (idx >= args.length) {
			if (mandatory) {
	...
	private @Nullable Long getArgAsLong(String[] args, int idx, String name, boolean mandatory)
		...
		try {
			...
			throw new CommandException("Bad value for %s. argument supplied for %s! Not a number!", idx, name);
		}
	}
	...
	private @Nullable GistFileRef toGistFileRef(@Nullable String gistFileRef) throws MalformedURLException {
