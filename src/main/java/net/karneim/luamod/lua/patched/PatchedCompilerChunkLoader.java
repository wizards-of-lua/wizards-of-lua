...
/**
 * A chunk loader that uses the {@linkplain LuaCompiler compiler} to convert Lua source text to Java
 * classfiles, and loads these classfiles into the VM using a {@link ClassLoader}.
 */
public class PatchedCompilerChunkLoader implements ChunkLoader, ExtendedChunkLoader {
  ...
  PatchedCompilerChunkLoader(ClassLoader classLoader, LuaCompiler compiler,
  ...
  /**
   * Returns a new instance of {@code CompilerChunkLoader} that uses the specified class loader
   * {@code classLoader} to load classes it compiles using {@code compiler}, with every main chunk
   * class having the class name {@code rootClassPrefix} followed by a monotonically-increasing
   * integer suffix.
   *
   ...
   *
   * @throws NullPointerException if {@code classLoader}, {@code compiler} or
   *         {@code rootClassPrefix} is {@code null}
   */
  public static PatchedCompilerChunkLoader of(ClassLoader classLoader, LuaCompiler compiler,
      String rootClassPrefix) {
  ...
  /**
   * Returns a new instance of {@code CompilerChunkLoader} that uses the specified class loader
   * {@code classLoader} to load classes it compiles using a new instance of the Lua compiler with
   * the settings {@code compilerSettings}, with every main chunk class having the class name
   * {@code rootClassPrefix} followed by a monotonically-increasing integer suffix.
   *
   * @param classLoader the class loader used by this chunk loader, must not be {@code null}
   ...
   * @return a new instance of {@code CompilerChunkLoader}
   *
   * @throws NullPointerException if {@code classLoader}, {@code compilerSettings} or
   *         {@code rootClassPrefix} is {@code null}
   */
  public static PatchedCompilerChunkLoader of(ClassLoader classLoader,
      CompilerSettings compilerSettings, String rootClassPrefix) {
    return new PatchedCompilerChunkLoader(classLoader, new LuaCompiler(compilerSettings),
        rootClassPrefix);
  }

  /**
   * Returns a new instance of {@code CompilerChunkLoader} that uses the specified class loader
   * {@code classLoader} to load classes it compiles using a compiler instantiated with
   * {@linkplain CompilerSettings#defaultSettings() default settings}, with every main chunk class
   * having the class name {@code rootClassPrefix} followed by a monotonically-increasing integer
   * suffix.
   *
   * @param classLoader the class loader used by this chunk loader, must not be {@code null}
   * @param rootClassPrefix the class name prefix for compiled classes, must not be {@code null}
  ...
  /**
   * Returns a new instance of {@code CompilerChunkLoader} that uses the class loader that loaded
   * the {@code CompilerChunkLoader} class to load classes it compiles using {@code compiler}, with
   * every main chunk class having the class name {@code rootClassPrefix} followed by a
  ...
  public static PatchedCompilerChunkLoader of(LuaCompiler compiler, String rootClassPrefix) {
    return of(PatchedCompilerChunkLoader.class.getClassLoader(), compiler, rootClassPrefix);
  }

  /**
   * Returns a new instance of {@code CompilerChunkLoader} that uses the class loader that loaded
   * the {@code CompilerChunkLoader} class to load classes it compiles using a new instance of the
   * Lua compiler with the settings {@code compilerSettings}, with every main chunk class having the
   * class name {@code rootClassPrefix} followed by a monotonically-increasing integer suffix.
   *
   * @param compilerSettings the compiler settings used to instantiate the compiler, must not be
   *        {@code null}
   * @param rootClassPrefix the class name prefix for compiled classes, must not be {@code null}
   * @return a new instance of {@code CompilerChunkLoader}
   *
   * @throws NullPointerException if {@code compilerSettings} or {@code rootClassPrefix} is
   *         {@code null}
   */
  public static PatchedCompilerChunkLoader of(CompilerSettings compilerSettings,
      String rootClassPrefix) {
  ...
  /**
   * Returns a new instance of {@code CompilerChunkLoader} that uses the class loader that loaded
   * the {@code CompilerChunkLoader} class to load classes it compiles using a compiler instantiated
   * with {@linkplain CompilerSettings#defaultSettings() default settings}, with every main chunk
   * class having the class name {@code rootClassPrefix} followed by a monotonically-increasing
   * integer suffix.
   *
   * @param rootClassPrefix the class name prefix for compiled classes, must not be {@code null}
  ...
  public LuaFunctionBinary compile(String chunkName, String sourceText) throws LoaderException {
    ...
    synchronized (this) {
      ...
      try {
        ...
        int line = 0; // TODO
        boolean partial = msg != null && msg.contains("Encountered: <EOF>"); // TODO: is there
                                                                             // really no better
                                                                             // way?
  ...
  @Override
  public LuaFunction loadTextChunk(Variable env, String chunkName, String sourceText)
  ...
  // @Override
  // public LuaFunction loadBinaryChunk(Variable env, String chunkName, byte[] bytes, int offset,
  // int len) throws LoaderException {
