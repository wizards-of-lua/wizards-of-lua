...
public class PacketPipeline extends MessageToMessageCodec<FMLProxyPacket, AbstractPacket> {
  ...
  private LinkedList<Class<? extends AbstractPacket>> packets =
      new LinkedList<Class<? extends AbstractPacket>>();
  ...
  public boolean registerPacket(Class<? extends AbstractPacket> clazz) {
    if (this.packets.size() > 256) {
      logger.error(String.format("Can't register packet %s. Number of packets exceeded 256."));
      return false;
    }

    if (this.packets.contains(clazz)) {
      logger.error(String.format("Can't register packet %s. Packet already registered."));
      return false;
    }

    if (this.isPostInitialized) {
      logger.error(String
          .format("Can't register packet %s. PacketPipeline has been already initialized."));
      return false;
    }
  ...
  protected void encode(ChannelHandlerContext ctx, AbstractPacket msg, List<Object> out)
    ...
    FMLProxyPacket proxyPacket =
        new FMLProxyPacket(new PacketBuffer(buffer.copy()), ctx.channel()
  ...
  public void postInitialize() {
    ...
    Collections.sort(this.packets, new Comparator<Class<? extends AbstractPacket>>() {
      ...
      public int compare(Class<? extends AbstractPacket> clazz1,
        ...
        int com =
            String.CASE_INSENSITIVE_ORDER.compare(clazz1.getCanonicalName(),
   ...
   * 
   * @param message The message to send
   * @param point The {@link cpw.mods.fml.common.network.NetworkRegistry.TargetPoint} around which
   *        to send
   */
