...
public class LivingDeathEventTest extends WolTestBase {
  ...
  public void test_cause_is_outOfWorld() throws Exception {
    ...
    // When:
    mc().executeCommand(
        "/lua q=Events.connect('LivingDeathEvent'); e=q:next(); print(e.cause)");
  ...
  public void test_entity_is_player() {
    ...
    try {
      mc().executeCommand(
          "/lua q=Events.connect('LivingDeathEvent'); e=q:next(); print(e.entity.name)");
      mc().executeCommand("/kill @a[name=testplayer]");

      // Then:
      ServerLog4jEvent act1 = mc().waitFor(ServerLog4jEvent.class);
      assertThat(act1.getMessage()).startsWith("testplayer fell out of the world");
      ServerLog4jEvent act2 = mc().waitFor(ServerLog4jEvent.class);
      assertThat(act2.getMessage()).startsWith("Killed testplayer");
      ServerLog4jEvent act3 = mc().waitFor(ServerLog4jEvent.class);
      assertThat(act3.getMessage()).isEqualTo("testplayer");
