...
properties:
  - name: autosleep
    ...
    Default is true normally, but in an event listener 'autosleep' is always false and can't be changed.
    See [Events.subscribe](/modules/Events#subscribe) for more information on event listeners.
    "
  - name: allowance
    type: number
    access: r
